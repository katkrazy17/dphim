...
abstract class BaseRepository 
{
    ...
    public function getRoute()
    {
        ...
        foreach ($this->all() as $value) {
            $folders = Arr::prepend($folders, $value['link_route']);
        }
        $route = [];
        foreach ($routes as $value) {
            if($value->getActionMethod() == 'index'|| $value->getActionMethod() == 'create' )
            {
                $route = Arr::prepend($route, $value->getName());
            }
        }
        return array_diff($route, $folders);
    }
    ...
    {
        return $this->_model->pluck($column1, $column2);
    }
